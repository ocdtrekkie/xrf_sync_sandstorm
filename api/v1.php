...
$new_ip_local=mysqli_real_escape_string($xrf_db, $_GET['ip_local'] ?? '');
...
if (mysqli_stmt_num_rows($identifysender) == 1)
{
	...
	$updatenode = mysqli_prepare($xrf_db, "UPDATE y_nodes SET last_seen = NOW(), last_ip_addr = ?, last_ip_local = ?, last_winver = ?, user_agent = ? WHERE access_key = ?");
	if ($user_agent == "") { $user_agent = mysqli_real_escape_string($xrf_db, $_SERVER['HTTP_USER_AGENT']); }
	mysqli_stmt_bind_param($updatenode, "sssss", $new_ip_addr, $new_ip_local, $new_winver, $user_agent, $access_key);
